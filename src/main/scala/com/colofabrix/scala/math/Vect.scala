...
case class XYVect(
  ...
  override
  def toString: String = s"Vec(x: ${x.sig() }, y: ${y.sig() })"
}
...
case class RTVect(
  ...
  override
  def ϑ: Double = restrictAngle(_ϑ)

  override
  def x: Double = ρ * Math.cos(ϑ)

  override
  def y: Double = ρ * Math.sin(ϑ)

  override
  def toString: String = s"Vec(ρ: ${ρ.sig() }, ϑ: ${ϑ.sig() })"
}
...
sealed
abstract
class Vect extends AnyRef with scalaz.Equal[Vect] {
  ...
  @inline
  protected
  def restrictAngle(angle: Double): Double = {
  ...
  override
  def equal(v1: Vect, v2: Vect): Boolean = v1.equals(v2)
}
