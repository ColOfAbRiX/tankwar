...
class PolygonTest extends ShapeTest[Polygon] {
  /**
    ...
    */
  override protected def testShape( bounds: Box ): Polygon = new Polygon( bounds.vertices )
  ...
  override protected def testShape( bounds: Box, touch: Double ): (Polygon, Vect) =
    (new Polygon( bounds.vertices ), bounds.topRight + (bounds.bottomRight - bounds.topRight) * touch)
}